...
function deps(overrides: Partial<SayDeps> = {}): SayDeps {
  return {
    ...
    addIdea: vi.fn(
      (): AddResult => ({
        ok: true,
        idea: { id: 7, uid: "u1", name: "Ana", text: "a cat", source: "voice", at: 0, kind: "new" },
      }),
    ),
...
describe("handleSay typed branch (parity with /say's original behaviour)", () => {
  ...
  it("passes the amend kind through to the queue", async () => {
    const d = deps();
    await handleSay(d, { uid: "u1", name: "Ana", text: "add a hat", source: "web", kind: "amend" });
    expect(d.addIdea).toHaveBeenCalledWith(expect.objectContaining({ kind: "amend" }));
  });
});
