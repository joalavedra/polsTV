...
Dev server: `pnpm dev` → http://localhost:4111. Static pages live in `src/mastra/public/`.
...
`name`: 1–24 chars. `text`: 1–280 chars.
...
| `GET /b/:secret/next-steer` | `200 { steerId, ideaId, prompt }` or `204`. Idempotent: the same steer is returned until resolved. Doubles as the broadcaster heartbeat, so poll it every ~3 s. |
